...
class UserProfile extends Component {
    ...
    fetch('http://localhost:8088/graphql', {
      ...
      .then(resData => {
        if (resData.errors) {
          ...
          this.setState({
            stripePromise: loadStripe(pocketVarsParsed.stripe.a)
          })
  ...
  addReminder = (event) => {
    ...
    let userIds1 = session.booked.map(x => x._id);
    const instructorIds = this.state.profileLessonViewerData.instructors.map(x => x._id);
    // let userIds2 = userIds1.push(instructorIds);
    let userIds2 = userIds1.concat(instructorIds);
    // let userIds3 = userIds2.toString();
    // let userIds4 = JSON.stringify(userIds2);
    // console.log(`
    //   userIds:
    //     0: ${JSON.stringify(session.booked)}
    //     1: ${instructorIds},
    //     2: ${userIds1},
    //     3: ${userIds2},
    //     4: ${userIds3},
            ...
            <Row>
              ...
              {this.state.user !== null && (
                  <ThisUserProfile
                    ...
                    stripePaid={this.state.stripePaid}
                    startStripeCheckout={this.startStripeCheckout}
                  />
                )}
